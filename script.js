...
let sliceIndex = null;
let currentQuestion; // This will hold the current question object
...
function generateQuestion() {
    const den = Math.floor(Math.random() * 8) + 3; // Denominator between 3 and 10
    let num1, num2, answer_num;

    if (currentMode === 'addition') {
        // Ensure num1 + num2 <= den
        num1 = Math.floor(Math.random() * (den - 1)) + 1; // num1 is at least 1
        num2 = Math.floor(Math.random() * (den - num1)) + 1; // num2 is at least 1
        answer_num = num1 + num2;
    } else { // Subtraction
        // Ensure num1 >= num2
        num1 = Math.floor(Math.random() * (den - 1)) + 2; // num1 is at least 2 to allow subtraction
        num2 = Math.floor(Math.random() * (num1 - 1)) + 1; // num2 is smaller than num1
        answer_num = num1 - num2;
    }
...
function loadQuestion() {
    ...
    document.getElementById('question-info').textContent = `Question ${currentQuestionIndex + 1} of ${questionsPerMode}`;
